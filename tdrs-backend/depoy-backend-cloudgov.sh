...
CGHOSTNAME_BACKEND="${CGHOSTNAME_BACKEND:-tdp-backend}"
...
if [ "$1" = "setup" ] ; then  echo
	...
	# if service_exists "tdp-backend-deployer" ; then
	#   echo tdp-backend-deployer already created
	# else
	#   cf create-service cloud-gov-service-account space-deployer tdp-backend-keys
	#   cf create-service-key tdp-backend-keys deployer
	#   echo "to get the CF_USERNAME and CF_PASSWORD, execute 'cf service-key tdp-backend-keys deployer'"
	# fi
